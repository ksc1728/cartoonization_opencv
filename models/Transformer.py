...
MODEL_PATH = os.path.join(os.path.dirname(__file__), 'pretrained_models1')


def get_available_styles():
    return ['Hayao', 'Hosoda', 'Paprika', 'Shinkai']



def get_model_filename(style):
   
    style_to_filename = {
        'Hayao': 'Hayao_net_G_float.pth',
        'Hosoda': 'Hosoda_net_G_float.pth',
        'Paprika': 'Paprika_net_G_float.pth',
        'Shinkai': 'Shinkai_net_G_float.pth'
    }
    return style_to_filename.get(style, f'{style}_net_G_float.pth')

def load_pretrained_model(style):
    assert style in get_available_styles(), f"Style {style} is not supported."

    # Create directory if it doesn't exist
    os.makedirs(MODEL_PATH, exist_ok=True)
    
    model = Generator()
    model_filename = get_model_filename(style)
    model_path = os.path.join(MODEL_PATH, model_filename)
    
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            f"Pretrained model for {style} not found at {model_path}\n"
            f"Expected filename: {model_filename}\n"
            f"Please ensure the model file exists in {MODEL_PATH}"
        )
